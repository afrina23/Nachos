...
StartProcess(const char *filename)
{
    ...
    processTable=ProcessTable::getInstance(10);
    ...
    int number=processTable->Alloc((void*)currentThread);
    currentThread->space = space;
    bool condition=space->Initialize(executable,number);
    currentThread->ID=number;

    if(!condition){
        printf("The program can't be loaded because there is no memory left");
